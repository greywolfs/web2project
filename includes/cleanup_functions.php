...
function w2PtoolTip($header = '', $tip = '', $raw = false, $id = '') {
    ...
    $id = ('' == $id) ? '' : 'id="' . $id . '"';
	if ($raw) {
		$starttip = '<span ' . $id . ' title="&lt;h4&gt;' . $AppUI->_($header) . '&lt;/h4&gt; ' . $AppUI->_($tip) . '">';
	} else {
		$starttip = '<span ' . $id . ' title="&lt;h4&gt;' . ucwords(strtolower($AppUI->_($header))) . '&lt;/h4&gt; ' . strtolower($AppUI->_($tip)) . '">';
	}
	return $starttip;
}
		...
		for ($i = 0, $i_cmp = $start->dateDiff($end); $i <= $i_cmp; $i++) {
			...
			if (($row['event_cwd'] && in_array($date->getDayOfWeek(), $cwd)) || !$row['event_cwd']) {
				if ($minical) {
					...
					$link['href'] = '';
					$link['alt'] = '';
					$link['text'] = w2PtoolTip($row['event_name'], getEventTooltip($row['event_id']), true) . w2PshowImage('event' . $row['event_type'] . '.png', 16, 16, '', '', 'calendar') . '</a>&nbsp;' . '<a href="' . $url . '"><span class="event">'. $start->format('%R'). ' - ' . $row['event_name'] . '</span></a>' . w2PendTip();
				}
